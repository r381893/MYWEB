// 請將此 URL 改成你自己的 Apps Script 部署網址
...
function submitMessage() {
  ...
  fetch(scriptURL, { method: "POST", body: formData })
    ...
    .catch(err => console.error("留言送出錯誤:", err));
}
  ...
  fetch(scriptURL)
    ...
    .then(data => {
      ...
      data.reverse().forEach(item => {
        container.innerHTML += `
          <div class="message">
            <img src="https://api.dicebear.com/7.x/fun-emoji/svg?seed=${item.name}" alt="avatar" class="avatar">
    ...
    .catch(err => console.error("讀取留言錯誤:", err));
}
